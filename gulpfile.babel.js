...
gulp.task('transpile:server:ts', [], () => {
    console.log('in here');
    return gulp.src(['server/**/!(*.spec|*.mock).ts', 'typings/index.d.ts', 'server/custom.d.ts'], { base: "." })
...
gulp.task('watch', () => {
    ...
    gulp.watch(paths.server.scriptsTs, ['transpile:server:ts']);
        // .pipe(plugins.plumber())
        // //.pipe(lintServerScripts())
        // .pipe(plugins.livereload());
...
gulp.task('serve', cb => {
    runSequence(['clean:tmp', 'constant', 'env:all', 'typings'],
        ['lint:scripts', 'inject'],
        ['wiredep:client'],
        ['transpile:client', 'transpile:server:ts', 'styles'],
